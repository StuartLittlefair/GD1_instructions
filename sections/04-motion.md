# Using proper motions to select GD-1 stars

In the previous episode, we wrote a query to select stars from the region of the sky where we expect GD-1 to be, and saved the results in a FITS and HDF5 file. Now we will read that data back in and implement the next step in the analysis, identifying stars with the proper motion we expect for GD-1.

We will answer the question - how do efficiently explore our data and identify appropriate filters to produce a clean sample (in this case of GD-1 stars)?
...
This imports previously written functions. `episode_functions.py` also includes the `make_dataframe` function written in the previous section.
...
```{figure} fig/gd1-1.png
:class: bg-white
:align: center
Proper motion in GD-1 coordinates
```
...
- Due to the nature of tidal streams, we expect the proper motion for
  ...
  motion in the direction of $\phi_2$ to be near 0.

- In the direction of $\phi_1$, we do not have a prior expectation for
...
Make a scatter plot of the proper motion in the GD-1 frame, for the stars in `results_df`.

Use `xlim` and `ylim` to zoom in on the same region near the origin as in the figure above.

Choose appropriate values for markersize and transparency.
```

Your plot should look something like the one below.

```{figure} fig/04-motion_files/04-motion_69_0.png
:class: bg-white
:align: center
```
...
Python's logical operators (`and`, `or`, and `not`) do not work with NumPy or Pandas.  Both libraries use the bitwise operators (`&`, `|`, and `~`) to do elementwise logical operations ([explanation here](https://stackoverflow.com/questions/21415661/logical-operators-for-boolean-indexing-in-pandas)).
```

Note that we need the parentheses around the conditions; otherwise the order of operations is incorrect.
...
This is the second time we are plotting proper motion, and we can imagine we might do it a few more times. Instead of copying and pasting the previous code, we will write a function that we can reuse on any dataframe. Feel free to change the markersize, transparency and colour settings in this function.
...
```{figure} fig/04-motion_files/04-motion_92_0.png
:class: bg-white
:align: center
```
...
Here are some suggested bounds on proper motion I chose by eye:
...
To draw these bounds, we will use the `make_rectangle` function we wrote earlier (and imported at the start of this notebook) to make two lists containing the coordinates of the corners of the rectangle.
...
![](fig/04-motion_files/04-motion_100_0.png)
...
Plot the $\phi_1$ and $\phi_2$ coordinates (the coordinates in the GD-1 frame) for the `selected_df`.
your plot should look a bit like the one below.
```

```{figure} fig/04-motion_files/04-motion_110_0.png
:class: bg-white
:align: center
```
...
Add the `axis` and `title` commands to the plotting code.
```

Before we go any further, we will put the code we wrote to make one of the panel figures into a function that we will use in future episodes to recreate this entire plot with a single line of code. It should look something like the one below.
...
At this point we have run a successful query and cleaned up the results. This is a good time to save the data. We have already started a results file called gd1\_data.hdf which we wrote `results_df` to.
...
We can add to our new Pandas `DataFrame` to an **existing** HDF5 file by omitting the `mode='w'` keyword like this:
...
We are going to need `centerline_df` later as well.  Write a line of code to add it as a third Dataset in the HDF5 file.
```

```{hint}
Since the file already exists, you should *not* use `mode='w'`.
```
...
We use a `with` statement here to open the file before the print statement and (automatically) close it after.  Read more about [context managers](https://book.pythontips.com/en/latest/context_managers.html).
```
...
- A data analysis is often prototyped on a small set of data which can be explored more easily and used to identify ways to limit a dataset to exactly the data you want.
- To store data from several Pandas `DataFrame`s, a good option is an HDF5 file, which can contain multiple Datasets.

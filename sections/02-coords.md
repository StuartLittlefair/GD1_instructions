# Working with Sky Coordinates

In the previous section, we wrote ADQL queries and used them to select and download data from the Gaia server. In this episode, we will write a query to select stars from a particular region of the sky.
...
The measurements we will work with are physical quantities, which means that they have two parts, a value and a unit. For example, the coordinate 30<sup>°</sup> has value 30 and its units are degrees.

Until recently, most scientific computation was done with values only; units were left out of the program altogether, [sometimes with catastrophic results](https://en.wikipedia.org/wiki/Mars_Climate_Orbiter#Cause_of_failure).
...
`Quantities` provides a method called `to` that converts to other units. For example, we can compute the number of arcminutes in `angle`:
...
- `CONTAINS`: a function that returns `1` if a `POINT` is contained in a shape and `0` otherwise. Here is the [documentation of `CONTAINS`](https://www.ivoa.net/documents/ADQL/20180112/PR-ADQL-2.1-20180112.html#tth_sEc4.2.12).

A query like this is called a cone search because it selects stars in a cone. Here is how we run it:
...
So we will start by selecting stars in a smaller rectangle near the center of GD-1, from -55 to -45 degrees for $\phi_1$ and -8 to 4 degrees for $\phi_2$. First we will learn how to represent these coordinates with Astropy.
...
Astropy provides a `SkyCoord` object that represents sky coordinates relative to a specified reference frame.

The following example creates a `SkyCoord` object that represents the approximate coordinates of [Betelgeuse](https://simbad.u-strasbg.fr/simbad/sim-basic?Ident=Betelgeuse) ($\alpha$ Ori) in the ICRS frame.

[ICRS](https://www.iers.org/IERS/EN/Science/ICRS/ICRS.html) is the "International Celestial Reference System", adopted in 1997 by the International Astronomical Union. It is the "standard" coordinate system for right ascension and declination that you have been using in your course so far.
...
To transform to and from GD-1 coordinates, we will use a frame defined by [Gala](https://gala-astro.readthedocs.io/en/latest/), which is an Astropy-affiliated library that provides tools for studying orbits in the Milky Way.

Gala provides [`GD1Koposov10`](https://gala-astro.readthedocs.io/en/latest/_modules/gala/coordinates/gd1.html), which is "a Heliocentric spherical coordinate system defined by the orbit of the GD-1 stream". In this coordinate system, one axis is defined along the direction of the steam (the x-axis in Figure 1) and one axis is defined perpendicular to the direction of the stream (the y-axis in Figure 1). These are called the $\phi_1$  and $\phi_2$  coordinates, respectively.
...
def make_rectangle(x1, x2, y1, y2):
    ...
    ----------
    x1, x2: float
      x-coordinates of left and right sides of rectangle, respectively
    y1, y2: float
      y-coordinates of top and bottom sides of rectangle, respectively

    Returns
    -------
    xcoords: list[float]
      a list of the x coordinates of the four corners of the rectangle. 
      The first corner is repeated
    ycoords: list[float]
      a list of the y coordinates of the four corners of the rectangle. 
      The first corner is repeated
    """
    xs = [x1, x1, x2, x2, x1]
    ys = [y1, y2, y2, y1, y1]
...
The return value is a tuple containing a list of coordinates in $\phi_1$ followed by a list of coordinates in $\phi_2$.
...
```{code-cell} python
corners = SkyCoord(phi1=phi1_rect, phi2=phi2_rect, 
                   frame=gd1_frame)
corners
```
...
We can use the Python string function `join` to join `corners_list_str` into a single string (with spaces between the pairs):
...
This is the set of stars we will work with in the next step.  Since we have a substantial dataset now, this is a good time to save it.

Storing the data in a file means we can shut down our notebook and pick up where we left off without running the previous query again.

Astropy `Table` objects provide a `write` function, which writes the table to disk.

```{note}
#### Filepaths
 
Since we want the results to still exist when we restart Colab, we should not save our file on the virtual computer created for our session. Instead, we should save it to our Google Drive. It makes sense to create a folder in your Google Drive called `GD1_Results`. The following code snippet assumes that you've created that folder - change the code if you use a different name or location!
```
...
In the next section, we'll reload the data from this file and replicate the next step in the analysis, using proper motion to identify stars likely to be in GD-1.

**You may wish to start a new notebook to work on the next section!**

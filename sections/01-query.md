...
In this lab, we will replicate parts of the analysis in the paper, "[Off the beaten path: Gaia reveals GD-1 stars outside of the main stream](https://arxiv.org/abs/1805.00425)" by Adrian Price-Whelan and Ana Bonaca. This paper traces stars from a Globular cluster that has been destroyed during a passage through the milky way, using the photometry and stellar motions of the stars.

In this first part, we will learn the following skills:
...
This section demonstrates the steps for selecting and downloading data
...
In order to select data from a database, you need to compose a query, which is a program written in a "query language". The query language we will use is ADQL, which stands for "Astronomical Data Query Language".
...
We will be working on **Google Colab** for this project. Some data files you will need are saved in a shared Google Drive with file path `/content/drive/Shareddrives/ClassroomShared/MPS216/GD1/`. We will also use from Python libraries that are not installed by default on Colab. You can install these by first connecting Google Colab to your Google Drive:
...
and then installing the Python modules with:
...
```{important}
Since Google Colab starts a new virtual computer each time you load a notebook, you will need to do this every time you open a notebook you are working on.
```

## Connecting to Gaia

The library we will use to get Gaia data is [Astroquery](https://astroquery.readthedocs.io/en/latest/). Astroquery provides `Gaia`, which is an [object that is used to query the Gaia database](https://astroquery.readthedocs.io/en/latest/gaia/gaia.html).

We can load the `Gaia` object from the astroquery library like this:
...
- `gaiadr2.panstarrs1_original_valid`, which contains the photometry data we will use from PanSTARRS, and
...
You might be wondering how we download these tables.  With tables this big, you generally don't! The Gaia DR `gaia_source` table has over 1.3 billion rows! Instead, you use queries to select only the data you want.
...
We use a [triple-quoted string](https://docs.python.org/3/tutorial/introduction.html#strings) here so we can include line breaks in the query, which makes it easier to read.
```
...
```{note}
#### Repetition in class names

Why is `table` repeated three times?  The first is the name of the module, the second is the name of the submodule, and the third is the name of the class.  Most of the time we only care about the last one.  It's like the Linnean name for the Western lowland gorilla, which is *Gorilla gorilla gorilla*.
```

An Astropy `Table` is similar to a table in an SQL database except:

- SQL databases are stored on disk drives, so they are persistent; that is, they "survive" even if you turn off the computer.  An Astropy `Table` is stored in memory; it disappears when you turn off the computer (or shut down your Jupyter notebook).
...
```{note}
Gaia parallaxes are in units of milli-arcseconds, so 1 mas is a distance of 1000 pc.
```
...
Your results for this query may differ from the Instructor's. This is because `TOP 3000` returns 3000 results, but those results are not organized in any particular way.
```
...
![](fig/Gaia-HR-diagram.jpeg)

Selecting stars with `bp-rp` less than 2 excludes many [class M dwarf stars](https://xkcd.com/2360/), which are low temperature, low luminosity.  A star like that at GD-1's distance would be hard to detect, so if it is detected, it is more likely to be in the foreground.
...
As an example, we will divide the previous query into two parts; a list of column names and a "base" for the query that contains everything except the column names.
...
And here is the base. It is a string that contains at least one format specifier in curly brackets (braces).
...
In this example, the variable that contains the column names and the variable in the format specifier have the same name. That is not required, but it is a common style.
...
This section has demonstrated the following steps:
...
In the next section we will extend these queries to select a particular region of the sky.
